...
function initMap() {
  ...
  const drinks = {
    location: {lat: 38.900034, lng: -77.021255},
    title: "Welcome Drinks",
    popupContent: '<h4>Welcome Drinks - Crimson Whiskey Bar</h4>'
  };
  ...
  const drinksInfoWindow = new google.maps.InfoWindow({
    content: drinks.popupContent,
  });

  const drinksMarker = new google.maps.Marker({
    position: drinks.location,
    map: map,
    title: 'Reception'
  });

  drinksMarker.addListener('click', function () {
    drinksInfoWindow.open(map, drinksMarker);
  });
